...
#myInput {
  ...
  <div class="column middle">
    ...
    <div class="main">
    <input type="text" id="myInput" placeholder=" Search for readymade design.." title="Type in a name">
           <div class="row">
           ...
           <?php
            ...
            $sql = "SELECT * FROM readymade where readymadeClass = 'Occasions' LIMIT $start_from, $limit";  
           ...
           <div class="column rm" id="">
               <form method="post" id="myForm" action="confirmation.php?id=<?php echo $row["readymadeID"] ?>&proId=<?php echo $proId ?>">
                    ...
                    <center><input type="submit" value="Choose" class="btn btn-primary"></center> 
                    </div></form></div>
                    <?php } ?>              
           </div>
...
$sql = "SELECT COUNT(readymadeID) FROM readymade where readymadeClass = 'Occasions'" ;  
$rs_result = mysqli_query($con, $sql);  
...
for ($i=1; $i<=$total_pages; $i++) { 
            $pagLink .= "<a href='occasionsDesign.php?page=".$i."&proId=". $proId ."'>".$i."</a>";  
};  
echo $pagLink . "</div>";  
?>
    </div>
