<?php
    ...
    $user_products_query="SELECT orderID, userID, it.productID, it.productName, it.productPrice, it.productImage, ut.orderQuantity, ut.orderPayment, ut.orderStatus, ut.orderDate, ut.orderTime, designType, rm.readymadeID, rm.readymadeImage, imageDesign from orders ut inner join product it on it.productID=ut.productID inner join readymade rm on rm.readymadeID=ut.readymadeID where ut.userID='$users_userID' and orderStatus = 'Pending' order by orderID DESC LIMIT $start_from, $limit";
...
<html>
    <head>
        ...
        <title>PrintShift | Order Pending</title>
        <meta charset="UTF-8">
...
?>
    ...
    <?php } ?>
        ...
        <hr>
            <h6>Order</h6>
        <hr>
            <p><a style="color: #00A693;" href = "checkStatus.php" >Order Pending</a> <span style="background-color:#00A693; padding: 2px 5px; color: white;"><?php echo $_SESSION['countOrder']; ?></span></p>
    ...
    <table>
        ...
        <tr>
            ...
            <?php if ($row['designType'] == 'Readymade' ){ ?>
                              ...
                              <?php echo '<img src="data:image;base64,'.base64_encode( $row['imageDesign'] ).'" style="width: 100px; height: 100px"> ' ?><?php } ?>
            </td>
            <td><?php echo $row['productName'];  ?></td>           
            <td>RM<?php echo $row['productPrice'];  ?></td>
            <td>x <?php echo $row['orderQuantity'];  ?></td>           
            <td><?php echo $row['orderDate'];  ?> <?php echo $row['orderTime'];  ?></td>
            <td>RM<?php echo $subTotal + 8 ?>.00</td>
            <td style="color: red;"><?php echo $row['orderStatus'];?></td>
        </tr>
...
$sql = "SELECT COUNT(orderID) FROM orders where userID = '".$_SESSION['users_userID']."' and orderStatus = 'Pending'" ;  
...
//$color = $_GET['page'];  
    if($i == 1):?>
    <a href='checkStatus.php?page=<?php echo $i;?>'><?php echo $i;?></a>
    <?php else:?>
    <a href='checkStatus.php?page=<?php echo $i;?>'><?php echo $i;?></a>
<?php endif;?>			
<?php endfor;endif; 
...
jQuery("#target-content").load("checkStatus.php?page=1");
	...
	e.preventDefault();
        ...
        var pageNum = this.id;
        jQuery("#target-content").load("checkStatus.php?page=" + pageNum);
    });
    });
</script>
